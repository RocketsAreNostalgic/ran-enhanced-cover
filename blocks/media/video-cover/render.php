...
if ( ! empty( $attributes['videoSources'] ) && is_array( $attributes['videoSources'] ) ) {
	foreach ( $attributes['videoSources'] as $source ) {
		...
		$source_url = esc_url( $source['url'] );

		if ( ! $source_url ) {
			continue;
		}

		$file_type   = wp_check_filetype( $source_url, wp_get_mime_types() );
		$source_type = ! empty( $file_type['type'] ) && str_starts_with( $file_type['type'], 'video/' )
			? $file_type['type']
			: 'video/mp4';
...
if ( $has_video ) {
	echo '<video class="ran-video-cover__media" muted loop playsinline preload="metadata" aria-hidden="true"' . ( $poster_url ? ' poster="' . esc_url( $poster_url ) . '"' : '' ) . ' style="object-position:' . esc_attr( $object_position ) . '">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Local escaped values only.
